...
merged_df = games_df.merge(
    player_stats_df, on="app_id", how="left").merge(
...
for idx, row in merged_df.iterrows():
    doc = {
        ...
        "player_stats": {
            ...
            "owners": row["owners"],
            "avg_playtime_min": float(row["avg_playtime_min"]) if not pd.isna(row["avg_playtime_min"]) else None,
            "ccu_at_1yr": int(row["ccu_at_1yr"]) if not pd.isna(row["ccu_at_1yr"]) else None,
        }, 
        "review_summary": {
            "metascore": float(row["metascore"]) if not pd.isna(row["metascore"]) else None,
            "user_score": int(row["user_score"]) if not pd.isna(row["user_score"]) else None
        }
    }
